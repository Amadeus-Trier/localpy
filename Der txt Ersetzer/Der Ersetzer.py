...
class WordReplacer(tk.Tk):
    ...
    def __init__(self):
        ...
        tk.Label(self, text="Wort was ersetzt wird").grid(row=0)
        self.word_to_replace = tk.Entry(self)
        self.word_to_replace.grid(row=0, column=1)

        tk.Label(self, text="Ersatzwort").grid(row=1)
        self.word_replacement = tk.Entry(self)
        self.word_replacement.grid(row=1, column=1)

        tk.Button(self, text='Schritt 1: Lade Datei', command=self.load_file).grid(row=3, column=1, pady=4)
        tk.Button(self, text='Schritt 2: Speicher Datei', command=self.save_file).grid(row=4, column=1, pady=4)
